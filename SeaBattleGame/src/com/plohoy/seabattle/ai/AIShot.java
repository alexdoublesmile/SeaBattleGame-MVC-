...
//				randomShot();
			...
			if (!ship.checkShipAlive()) {
				for (Cell cell : ship.getAroundCells()) {
					if (!checkCellIsShot(cell)) {
						System.out.println("-------------------------------------------------------------------------");
						System.out.println("----------------- формируем территорию вокруг вновь потопленного корабл€ - "
								+ n + " клеточек");
						System.out
								.println("---------------------------------------------------------------------------");
						n++;
	...
	public boolean cellTerritoryIsNotThisCell(Cell cell, int xCell, int yCell) {
		if (cell.getXCoord() == cell.getXCoord() + xCell && cell.getYCoord() == cell.getYCoord() + yCell) {
			return false;
		}
		return true;
	}
	...
	public boolean cellTerritoryIsNotOutOfField(Cell cell, int xCell, int yCell) {
		if (cell.getXCoord() + xCell < 0 || cell.getXCoord() + xCell > fieldSize - 1 || cell.getYCoord() + yCell < 0
				|| cell.getYCoord() + yCell > fieldSize - 1) {
			return false;
		}
		return true;
	}
	...
	public void setShotDownCellTerritory(Cell cell) {
		if (mainAxisIsY) {
			...
			for (int yCell = -1; yCell < 2; yCell++) {
				if (cellTerritoryIsNotThisCell(cell, 0, yCell) && cellTerritoryIsNotOutOfField(cell, 0, yCell)
			...
			int n = 1;
			for (Cell cellTerr : cellTerritory) {
				System.out.println("ячейка территории є" + n + ": (" + (cellTerr.getXCoord() + 1) + ", "
						+ (cellTerr.getYCoord() + 1) + ")");
				n++;
			}
			System.out.println("----------------------------------------------------------");
			...
			for (int xCell = -1; xCell < 2; xCell++) {
				if (cellTerritoryIsNotThisCell(cell, xCell, 0) && cellTerritoryIsNotOutOfField(cell, xCell, 0)
	...
	public void setExtremeShotDownCellsTerritoty(Ship ship) {
		...
		for (Cell cell : cellTerritory) {
			System.out.println(
					"ячейка территории є" + n + ": (" + (cell.getXCoord() + 1) + ", " + (cell.getYCoord() + 1) + ")");
			n++;
		}
		System.out.println("----------------------------------------------------------");
...
//		removeShotsFromTerritory();
		...
		for (Cell cell : cellTerritory) {
			System.out.println(
					"ячейка территории є" + num + ": (" + (cell.getXCoord() + 1) + ", " + (cell.getYCoord() + 1) + ")");
			num++;
		}
	...
	public Cell findFirstExtremeShotDownCell(Ship ship) {
		...
		System.out.println(
				"ячейка: (" + (firstExtremeCell.getXCoord() + 1) + ", " + (firstExtremeCell.getYCoord() + 1) + ")");
	...
	public Cell findLastExtremeShotDownCell(Ship ship) {
		...
		System.out.println(
				"ячейка: (" + (lastExtremeCell.getXCoord() + 1) + ", " + (lastExtremeCell.getYCoord() + 1) + ")");
						...
						if (thirdCell.getXCoord() - 1 == secondCell.getXCoord()
							...
							for (Cell fourthCell : copyFieldTerritory) {
								if (fourthCell.getXCoord() - 1 == thirdCell.getXCoord()
									...
									System.out.println("ячейка дл€ 4-палубника: (" + (firstcell.getXCoord() + 1) + ", "
											+ (firstcell.getYCoord() + 1) + ")");
									System.out.println("ячейка дл€ 4-палубника: (" + (secondCell.getXCoord() + 1) + ", "
											+ (secondCell.getYCoord() + 1) + ")");
									System.out.println("ячейка дл€ 4-палубника: (" + (thirdCell.getXCoord() + 1) + ", "
											+ (thirdCell.getYCoord() + 1) + ")");
									System.out.println("ячейка дл€ 4-палубника: (" + (fourthCell.getXCoord() + 1) + ", "
											+ (fourthCell.getYCoord() + 1) + ")");
									switch (n) {
									case (1):
				...
				if (secondCell.getXCoord() - 1 == firstcell.getXCoord()
					...
					for (Cell thirdCell : fieldTerritory) {
						if (thirdCell.getXCoord() - 1 == secondCell.getXCoord()
							...
							System.out.println("ячейка дл€ 3-палубника: (" + (firstcell.getXCoord() + 1) + ", "
									+ (firstcell.getYCoord() + 1) + ")");
							System.out.println("ячейка дл€ 3-палубника: (" + (secondCell.getXCoord() + 1) + ", "
									+ (secondCell.getYCoord() + 1) + ")");
							System.out.println("ячейка дл€ 3-палубника: (" + (thirdCell.getXCoord() + 1) + ", "
									+ (thirdCell.getYCoord() + 1) + ")");
		...
		for (Cell firstcell : fieldTerritory) {
			for (Cell secondCell : copyFieldTerritory) {
				if (secondCell.getXCoord() - 1 == firstcell.getXCoord()
					...
					System.out.println("ячейка дл€ 2-палубника: (" + (firstcell.getXCoord() + 1) + ", "
							+ (firstcell.getYCoord() + 1) + ")");
					System.out.println("ячейка дл€ 2-палубника: (" + (secondCell.getXCoord() + 1) + ", "
							+ (secondCell.getYCoord() + 1) + ")");
