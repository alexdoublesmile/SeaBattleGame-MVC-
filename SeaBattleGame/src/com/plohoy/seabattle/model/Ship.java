...
public class Ship {
	...
	public Ship(int x, int y) {
		...
		createShipTerritory(Player.FIELD_SIZE);
	}
	...
	public boolean checkShipOutOfField(int bottom, int top) {
		for (Cell cell : cells) {
			if (cell.getXCoord() < bottom || cell.getXCoord() > top || cell.getYCoord() < bottom
					|| cell.getYCoord() > top) {
				return true;
			}
		}
	...
	public boolean checkShipTouch(Ship ship) {
		for (Cell cell : cells) {
			if (cell.checkCellTouch(ship.getCells())) {
				return true;
			}
		}
		return false;
